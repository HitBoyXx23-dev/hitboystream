...
let titleDocument = function(e) {
  ...
  , show_id = window.location.hash.substring(2).toLowerCase().split("/").pop();
show_id && 0 !== show_id.length && themoviedb(`tv/${show_id}?language=${getTmdbLanguage()}&append_to_response=external_ids`).then(e => e.json()).then(d => {
    titleDocument(d),
    ...
    d.seasons.filter(e => 0 !== e.season_number).filter(e => !inFuture(e.air_date)).forEach(e => {
        var t = document.createElement("div")
          , a = (t.classList.add("videoItem"),
        document.createElement("a"))
          , n = (a.classList.add("thumbnail"),
        a.setAttribute("href", `../view/#/${tvSlugify(d.name, d.original_name)}/${show_id}/` + e.id),
