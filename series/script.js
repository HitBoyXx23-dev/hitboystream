...
async function get(e) {
    return themoviedb(`trending/tv/week?language=${getTmdbLanguage()}&page=` + e).then(e => e.json()).then(e => e.results)
}
function getImage(e) {
...
function create(e, t) {
    ...
    themoviedb(`tv/${e.id}?language=` + getTmdbLanguage()).then(e => e.json()).then(e => {
        var t, a;
...
searchInput.addEventListener("input", async () => {
    ...
    searchTimeout = setTimeout( () => {
        themoviedb(`search/tv?query=${searchInput.value.trim()}&include_adult=false&language=` + getTmdbLanguage()).then(e => e.json()).then(async e => {
